...
public class httpRequester implements Serializable {
  ...
  public List<RoughFeed> getFeeds() {
    ...
    JavaRDD<SingleSubscription> subscriptions =
        sc.parallelize(this.subscription.getSubscriptionsList());
    JavaRDD<RoughFeed> roughFeeds =
        subscriptions.flatMap(
            singleSubscription -> {
              ...
              for (int i = 0; i < singleSubscription.getUrlParamsSize(); i++) {
                ...
                if (feed != null) {
                  RoughFeed roughFeed =
                      new RoughFeed(urlType, singleSubscription.getUrlParams(i), feed);
